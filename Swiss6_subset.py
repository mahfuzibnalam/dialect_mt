...
Wynigen,BE;Zell,LU;Zermatt,VS;Ziefen,BL;Zofingen,AG;Zürich,ZH;Zug,ZG;Zunzgen,BL;Zweisimmen,BE"

regions = "AG;BE;SO;BL;VS;OW;SZ;UR;SG;TG;ZH;AI;GR;BS;SH;AR;NW;LU;FR;GL;ZG"

final_indexes = []
for i, dialect in enumerate(dialects.split(";")):
    filename = f"data-{args.lang}/dialects/{dialect}.{args.lang_code}"
...
for typ in ["region"]:
    if typ == "region":
        for mtmodel in ["nllb-200-distilled-600M", "nllb-200-distilled-1.3B", "nllb-200-1.3B", "nllb-200-3.3B"]:
            ...
            dialecttoid = {}
            for id, dialect in enumerate(dialects.split(";")):
                dialecttoid[dialect] = id

            with open(f"scores/{args.lang}/COMET_dialects_avg_{mtmodel}.txt", "r") as r:
                scores = r.read().splitlines()

            with open(f"scores/{args.lang}/COMET_regions_avg_{mtmodel}.txt", "w") as ww:
                for all in alltodialect:
                    average = 0.0
                    cnt = 0
                    for dialect in alltodialect[all]:
                        if dialect in dialecttoid:
                            index = dialecttoid[dialect]
