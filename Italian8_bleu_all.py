...
comunes = "Albosaggia;Aldeno;Altare;Arcola;Arenzano;Ne;Bergantino;Bologna;Bondeno;Borgofranco d'Ivrea;Borgomanero;Calizzano;Casalmaggiore;\
...
bleu = BLEU()
for model in ["DeltaLM", "nllb-200-distilled-600M", "nllb-200-distilled-1.3B", "nllb-200-1.3B", "nllb-200-3.3B"]:
    for typ in ["region", "province", "comune"]:
        if typ == "region":
            alls = regions
            id = -1
        if typ == "province":
        ...
        with open(f"data-Italian/dialects_{model}_translation/ref.eng", "r") as r:
        ...
        os.system(f"mkdir scores/Italian/BLEU/dialects_{model}_translation")
        with open(f"scores/Italian/BLEU_{typ}s_avg_{model}.txt", "w") as ww:
            for all in alltodialect:
                avg = 0.0
                cnt = 0
                for dialect in alltodialect[all]:
                    filename = f"data-Italian/dialects_{model}_translation/{dialect}.eng"
