...
for mtmodel in ["nllb-200-distilled-600M", "nllb-200-distilled-1.3B", "nllb-200-1.3B", "nllb-200-3.3B"]:
    os.system(f"mkdir scores/Italian/COMET/dialects_{mtmodel}_translation")
    with open(f"scores/Italian/COMET_dialects_avg_{mtmodel}.txt", "w") as ww:
        for dialect in dialects.split(";"):
            dialect = dialect.replace(" ", "_")
            filename = f"data-Italian/dialects_{mtmodel}_translation/{dialect}.eng"
            with open(filename, "r") as r:
                lines = r.read().splitlines()
            with open(f"data-Italian/dialects_{mtmodel}_translation/ref.eng", "r") as r:
                rlines = r.read().splitlines()
            

            filename = filename.replace("data-Italian", "scores/Italian/COMET")
