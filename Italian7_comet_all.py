...
for model in ["DeltaLM", "nllb-200-distilled-600M", "nllb-200-distilled-1.3B", "nllb-200-1.3B", "nllb-200-3.3B"]:
    for typ in ["comune", "province", "region"]:
        if typ == "region":
            alls = regions
            id = -1
        if typ == "province":
        ...
        with open(f"scores/Italian/COMET_{typ}s_avg_{model}.txt", "w") as ww:
            for all in alltodialect:
                avg = 0.0
                cnt = 0
                for dialect in alltodialect[all]:
                    filename = f"scores/Italian/COMET/dialects_{model}_translation/{dialect}.eng"
