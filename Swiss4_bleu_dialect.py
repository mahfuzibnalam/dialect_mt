...
for model in ["nllb-200-distilled-600M", "nllb-200-distilled-1.3B", "nllb-200-1.3B", "nllb-200-3.3B"]:
    for typ in ["dialect", "region"]:
        if typ == "region":
            ...
            os.system(f"mkdir scores/{args.lang}/BLEU/dialects_{model}")
            with open(f"scores/{args.lang}/BLEU_{typ}s_avg_{model}.txt", "w") as ww:
                for all in alltodialect:
                    avg = 0.0
                    cnt = 0
                    for dialect in alltodialect[all]:
                        filename = f"data-{args.lang}/dialects_{model}/{dialect}.eng"
                        with open(filename, "r") as r:
                            lines = r.read().splitlines()
                        with open(f"data-{args.lang}/dialects_{model}/{dialect}_standard.eng", "r") as r:
